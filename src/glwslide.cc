...
static void glwSliderLayout(GLWLayoutManager *_this,GLWSlider *_slider){
 if(_slider->ornt==GLWC_VERTICAL){
  ...
  if(_slider->labels.size>0&&_slider->center_labels){
   const char **lblp;
   lblp=(const char **)htGet(&_slider->labels,&_slider->label_lo);
   if(lblp!=NULL&&*lblp!=NULL){
    _slider->track_offs=(glwFontGetStringWidth(_slider->super.font,*lblp)>>1)+
                        GLW_SLIDER_INSET;}
   lblp=(const char **)htGet(&_slider->labels,&_slider->label_hi);
   if(lblp!=NULL&&*lblp!=NULL){
    w=(glwFontGetStringWidth(_slider->super.font,*lblp)>>1)+GLW_SLIDER_INSET;}
...
void glwSliderPeerDisplay(GLWSlider *_this,GLWCallbacks *_cb){
 ...
 if(_this->ornt==GLWC_VERTICAL){
  ...
  if(_this->labels.size>0){                                  /*Draw the labels*/
   ...
   while(hiInc(&ci)){
    vp=(int *)hiGetKey(&ci);
    lblp=(const char **)hiGetValue(&ci);
    if(vp!=NULL&&lblp!=NULL&&*lblp!=NULL){
     double cy;
     ...
     glwFontDrawString(_this->super.font,*lblp,cx,cy+dy);} } } }
 else{
  int cx,cy,cw;
  ...
  if(_this->labels.size>0){                                  /*Draw the labels*/
   ...
   while(hiInc(&ci)){
    ...
    if(vp!=NULL&&lblp!=NULL&&*lblp!=NULL){
     ...
     glwFontDrawString(_this->super.font,*lblp,cx,cy);} } } }
...
static int glwSliderPeerSpecial(GLWSlider *_this,GLWCallbacks *_cb,
 ...
 if(!_this->super.mouse_b){
  ...
  switch(_k){
   case GLUT_KEY_LEFT     :
   case GLUT_KEY_DOWN     :{
    glwSliderSetVal(_this,_this->val+dir,_this->ext);}break;
   case GLUT_KEY_RIGHT    :
   case GLUT_KEY_UP       :{
    glwSliderSetVal(_this,_this->val-dir,_this->ext);}break;
   case GLUT_KEY_PAGE_DOWN:glwSliderScrollByBlock(_this,dir);break;
   case GLUT_KEY_PAGE_UP  :glwSliderScrollByBlock(_this,-dir);break;
   case GLUT_KEY_HOME     :glwSliderSetVal(_this,_this->min,_this->ext);break;
   case GLUT_KEY_END      :glwSliderSetVal(_this,_this->max,_this->ext);break;
   default                :ret=0;} }
 else ret=0;
...
static void glwSliderPeerDispose(GLWSlider *_this,GLWCallbacks *_cb){
 ...
 hiInit(&hi,&_this->labels);
 while(hiInc(&hi)){
  char **lblp;
  lblp=(char **)hiGetValue(&hi);
  if(lblp!=NULL)free(*lblp);}
 htDstr(&_this->labels);}
...
void glwSliderInit(GLWSlider *_this,int _min,int _max,int _val,int _ext){
 ...
 _HTInit(&_this->labels,0,int,const char *,NULL,NULL);
 _this->changed=NULL;
...
int glwSliderDelLabel(GLWSlider *_this,int _val){
 char *lbl;
 if(htDel(&_this->labels,&_val,&lbl)){
  free(lbl);
