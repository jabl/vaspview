 ...
 if(_this->super.bounds.w>0&&_this->super.bounds.h>0){
  ...
  switch(_this->proj){
   case DS3V_PROJECT_ORTHOGRAPHIC:{
    ...
    else vectSet3d(_track,dx,dy,_this->track_rt*sqrt(d));}break;
...
static int ds3ViewAxesPeerSpecial(DS3ViewComp *_this,const GLWCallbacks *_cb,
 ...
 if(!(mods&GLUT_ACTIVE_CTRL))switch(_k){
  case GLUT_KEY_LEFT     :axis=-2;break;
  case GLUT_KEY_RIGHT    :axis=2;break;
  case GLUT_KEY_UP       :axis=-1;break;
  case GLUT_KEY_DOWN     :axis=1;break;
 ...
 if(ret<0){
  ...
  ret=-1;
  switch(_k){
   case GLUT_KEY_LEFT     :axis=1;break;
   case GLUT_KEY_RIGHT    :axis=-1;break;
   case GLUT_KEY_DOWN     :axis=2;break;
   case GLUT_KEY_UP       :axis=-2;break;
   case GLUT_KEY_PAGE_DOWN:axis=3;break;
   case GLUT_KEY_PAGE_UP  :axis=-3;break;
   case GLUT_KEY_HOME     :{
...
static int ds3ViewBoxPeerSpecial(DS3ViewComp *_this,const GLWCallbacks *_cb,
 ...
 switch(_k){
  case GLUT_KEY_LEFT     :{
   ...
   glwCompRepaint(&view->super,0);}break;
  case GLUT_KEY_RIGHT    :{
   view->track_pl-=2;
   if(view->track_pl<0)view->track_pl=3-view->track_pl;
   glwCompRepaint(&view->super,0);}break;
  case GLUT_KEY_UP       :dx=DS3V_UNIT_DIST;break;
  case GLUT_KEY_PAGE_UP  :dx=10*DS3V_UNIT_DIST;break;
  case GLUT_KEY_DOWN     :dx=-DS3V_UNIT_DIST;break;
  case GLUT_KEY_PAGE_DOWN:dx=-10*DS3V_UNIT_DIST;break;
  case GLUT_KEY_HOME     :{
   dx=(view->track_pl&1)-
      view->box[view->track_pl&1][view->track_pl>>1];}break;
  case GLUT_KEY_END      :{
